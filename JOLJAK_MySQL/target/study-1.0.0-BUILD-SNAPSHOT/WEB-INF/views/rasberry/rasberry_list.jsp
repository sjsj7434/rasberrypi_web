	...
	<div align=center>
			...
			chartDrow : function() {
				...
				function drawDashboard() {
					...
					for (var i = 0; i <= 5; i++) { //랜덤 데이터 생성
						...
						var ranNum = Math.floor(Math.random() * 300) + 1;
						var man = Math.floor(Math.random() * ranNum) + 1;
						var woman = ranNum - man;
						
						console.log('ran:'+ranNum);
						//console.log('temp:'+${temp[k].temp}');
						k++;
	
						dataRow = [ new Date('2020', '09', i, '10'), man, woman ];
						data.addRow(dataRow);
					}
		...
		<table border="1">
		  ...
		  <c:forEach var="temp" items="${temp}" >
			  <tr>
			  	<td><c:out value="${temp.time}"/></td>
			    <td><c:out value="${temp.temp}"/></td>
			    <td><c:out value="${temp.hud}"/></td>
			  </tr>
		  </c:forEach>
		</table>
	</div>
